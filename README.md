...
- **Работоспособность API**: OpenAI, Claude, Gemini, Perplexity, Grok, Ideogram.
- **Даты пополнения** по каждому сервису — хранятся локально в `storage.json`, удобно сверять с расходом.
- **Расписание**: ежедневный полный отчёт (время задаётся в `.env`), периодические отчёты только по балансам, опциональные алерты при низком балансе (USD, токены Suno, символы ElevenLabs, кредиты Runway через порог в $).
